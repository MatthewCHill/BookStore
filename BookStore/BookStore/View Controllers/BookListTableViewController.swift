...
class BookListTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.updateViews(bookDisplayed: book)
        return cell
    }
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            BookController.sharedInstance.deleteBook(bookToDelete: bookToDelete)
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "bookCell" {
            // what cell?
            if let indexPath = tableView.indexPathForSelectedRow {
                // what destination
                if let destinationVC = segue.destination as? BookDetailViewController {
                    // object to send
                    let bookToSend = BookController.sharedInstance.books[indexPath.row]
                    //object to recieve
